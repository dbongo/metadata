...
object Auth extends Controller with MongoController with Secured {
   ...
   */
  implicit class ResultWithSessionToken(result: SimpleResult) {
    /*
...
trait Secured extends Controller {
  ...
  object WithAuth {
    ...
    def apply(f: JsObject => Request[AnyContent] => Future[SimpleResult]): Action[AnyContent] = WithAuth(parse.anyContent)(f)

    def apply[T](p: BodyParser[T])(f: JsObject => Request[T] => Future[SimpleResult]) =
   ...
   */
  object WithUser {
    def apply(getUser: JsValue => Future[Option[JsObject]])(withoutUser: Request[JsValue] => Future[SimpleResult] = _ => Future.successful(NotFound))(withUser: JsObject => Request[JsValue] => Future[SimpleResult]) =
    ...
    // with email pattern
    def withEmail(withoutUser: Request[JsValue] => Future[SimpleResult] = _ => Future.successful(NotFound))(withUser: JsObject => Request[JsValue] => Future[SimpleResult]): Action[JsValue] =
      WithUser(findUserByEmail(_))(withoutUser)(withUser)

    // with email, password pattern
    def withEmailPassword(withoutUser: Request[JsValue] => Future[SimpleResult] = _ => Future.successful(NotFound))(withUser: JsObject => Request[JsValue] => Future[SimpleResult]): Action[JsValue] =
